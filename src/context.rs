...
use crate::{
    ...
    ServerError, ServerResult, ThreadedModel, UploadValue, Value,
};
...
pub trait DataContext<'a> {
    ...
    fn data<D: Any + ThreadedModel>(&self) -> Result<&'a D>;
...
impl Data {
    /// Insert data.
    pub fn insert<D: Any + ThreadedModel>(&mut self, data: D) {
...
impl<'a, T> ContextBase<'a, T> {
    ...
    ///
    /// Returns a `Error` if the specified type data does not exist.
    pub fn data<D: Any + ThreadedModel>(&self) -> Result<&'a D> {
    ...
    ///
    /// It will panic if the specified data type does not exist.
    pub fn data_unchecked<D: Any + ThreadedModel>(&self) -> &'a D {
        self.data_opt::<D>()
    ...
    /// Gets the global data defined in the `Context` or `Schema` or `None` if
    /// the specified type data does not exist.
    pub fn data_opt<D: Any + ThreadedModel>(&self) -> Option<&'a D> {
        self.query_env
            .extension_data
            .0
            .get(&TypeId::of::<D>())
